" echomsg will print the messages in the command line
...
function! test#kotlin#maventest#test_file(file) abort
    if exists('g:test#kotlin#runner') && g:test#kotlin#runner ==# 'maventest'
        return 1
    elseif exists("g:test#kotlin#runner") && g:test#kotlin#runner != 'maventest'
        return 0
    endif
...
function! test#kotlin#maventest#build_position(type, position) abort
  ...
  let package = s:get_kotlin_package(a:position['file'])
  ...
  if a:type ==# 'nearest'
    ...
    if !empty(name)
      return [test_cmd . package . '.' . filename . name ]
    else
...
function! s:get_kotlin_package(filepath)
  ...
  let relpath = substitute(abspath, '^.*src/\(main\|test\)/\(kotlin/\)\?', "", "g")
  let package_path = substitute(relpath, '\/[^/]\+$', "", "g")
  let kotlin_package = substitute(package_path, '/', '.', 'g')
